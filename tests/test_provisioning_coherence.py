...
def test_randomization_platform_role_contains_minimal_real_provisioning_contract() -> None:
    randomization_tasks_file = REPO_ROOT / "provisioning" / "roles" / "randomization_platform" / "tasks" / "main.yml"
    randomization_defaults_file = REPO_ROOT / "provisioning" / "roles" / "randomization_platform" / "defaults" / "main.yml"

    tasks_content = randomization_tasks_file.read_text(encoding="utf-8")
    defaults_content = randomization_defaults_file.read_text(encoding="utf-8")

    # Contract: role is real (not no-op) and manages runtime/service artifacts.
    assert "Install minimal randomization platform runtime dependencies" in tasks_content
    assert "Deploy randomization platform script artifact" in tasks_content
    assert "Configure randomization platform systemd unit" in tasks_content

    # Contract: source artifact comes from repository script helper.
    assert "subcase_1b/scripts/randomization_platform_start.sh" in defaults_content


def test_router_role_stays_noop_only_in_playbook_contract() -> None:
    roles_by_host = _extract_play_roles_by_host()

    router_roles = roles_by_host.get("router", [])
    assert router_roles == ["router_noop"], (
        "Router host must stay mapped only to router_noop to keep no-op intent explicit"
    )
